...
func login(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "GET" {
		...
		client, err := mongo.NewClient(options.Client().ApplyURI("mongodb+srv://mydb:Orange5Cat@cluster0.yklnk.mongodb.net/test"))
		if err != nil {
			log.Fatal(err)
		}
		...
		pass := generate(r.Form["password"][0])
		docs := []interface{}{
...
func users_func(w http.ResponseWriter, r *http.Request) {
	fmt.Println("DF")
	r.ParseForm() //Parse url parameters passed, then parse the response packet for the POST body (request body)
	x := r.URL.Path
	x = x[7:]
...
func main() {
	http.HandleFunc("/", user) // setting router rule
	http.HandleFunc("/users", login)
	http.HandleFunc("/users/", users_func)
